...
describe('DockerService managed specs ↔ docker-compose.full.yml parity', () => {
  ...
  const NOT_FORWARDED: Record<string, Record<string, string>> = {
    'docker-compose.yml': {
      ...
      // This stack's production compose is local-filesystem-only storage (see the STORAGE_TYPE
      // comment above it); an operator who needs S3/MinIO runs docker-compose.dev.yml, which does
      // forward these.
      S3_ENDPOINT: 'local-storage-only production compose',
      S3_BUCKET: 'local-storage-only production compose',
      S3_REGION: 'local-storage-only production compose',
      S3_ACCESS_KEY_ID: 'local-storage-only production compose',
      S3_SECRET_ACCESS_KEY: 'local-storage-only production compose',
...
describe('PostgreSQL client is not older than the servers the stack ships', () => {
  ...
  it('the Dockerfile client covers both the compose server and the managed-container server', () => {
    ...
    // docker-compose.full.yml, not docker-compose.yml: this fork's default compose has the bundled
    // Postgres/MinIO containers deliberately removed (see the parity describe block above), so the
    // version pin lives only in the full variant.
    const composeServer = major('docker-compose.full.yml', /image:\s*postgres:(\d+)/);
