...
export class AuthService implements OnModuleInit, OnModuleDestroy {
   ...
   */
  async createApiKey(dto: CreateApiKeyDto, selfServiceEmail?: string): Promise<{ apiKey: ApiKey; rawKey: string }> {
  ...
  async reassignSessionOwnership(fromApiKeyId: string, toApiKeyId: string): Promise<number> {
    const result = await this.sessionRepository.update(
      { createdByApiKeyId: fromApiKeyId },
      { createdByApiKeyId: toApiKeyId },
    );
