...
- Cherry-picked 22 more reliability fixes from upstream `rmyndharis/OpenWA`, round 2 of the process from PR #7 (session auto-start no longer blocking the HTTP listener; a stop/delete of an unknown session id no longer leaking a teardown-mark entry; plugin reinstall/uninstall recovery plus legacy-plugins-directory tracking; Baileys stickers converted to WebP instead of being published as mislabelled bytes; a bare participant phone number qualified on the whatsapp-web.js membership-request routes; `AUDIT_RETENTION_DAYS` boot-validated as a signed integer, since `<= 0` is a documented "disable retention" spelling; `.env.example` no longer shipping `ENABLE_SWAGGER=true` uncommented next to `NODE_ENV=production`; a bare `@SkipThrottle()` actually exempting `/api/metrics` and `/api/health*`; the chat-media retention/orphan sweeps running while archiving is off; a webhook's `filters` published as nullable in the OpenAPI contract; four SDK fixes — the JavaScript client's production error-envelope decode, the JS/Python/Go/Java chat-history field gaps, the Go client's field-clearing bug on webhook/template updates, and the Python `list`-return type annotations that resolved to the same-named method instead of the builtin; three Helm chart fixes — one ServiceMonitor scrape target per pod, a startup probe so the kubelet stops killing a booting pod, and a config-change rollout via checksums; and a checkout-path-with-spaces crash in three build/CI scripts). Excluded this round: fixes that still depend on the whatsapp-web.js participant-arity patcher or the Baileys newsletter-create patcher, neither of which this fork has adopted; fixes layered on features this fork hasn't taken — `quotedMessageId` on sends, chat pin/mute, channel-admin demote/ownership-transfer, and the SDK exposure of group-membership-requests, own-presence, call-link, blocked-contacts and channel-admin; a single 86-commit `fix/hardening-and-contract-corrections` branch mixing feature work, refactors and self-corrections across plugins, storage, takeover and ingress, excluded wholesale as too large and interdependent to cherry-pick piecemeal with confidence; and CI/tooling gates this fork doesn't run (an npm-audit per-advisory allowlist, an SDK-coverage wildcard check, a Python mypy gate). See the individual `git log` entries (`git log --grep "(cherry picked from commit"`) for exact provenance.
