...
- Cherry-picked 15 reliability fixes from upstream `rmyndharis/OpenWA` (engine media-burst shedding/hangs, WA-Web version resolve reporting, avatar lookup misreporting, wwjs pairing-code timing, participant/WID validation across group and Baileys writes, stale full-text search index, and import-data payload validation). 5 related upstream fixes were left out because they depend on a whatsapp-web.js participant-arity patcher and channel-admin feature work this fork hasn't taken; see the individual `git log` entries (`git log --grep "(cherry picked from commit"`) for exact provenance.
- The chart's optional ServiceMonitor now scrapes one target per pod instead of two. Its selector matched only the shared selector labels, which both the ClusterIP and the headless Service carry, so Prometheus discovered a target through each and produced two sets of series distinguished only by the `service` label — enough to double-count any naive aggregation. The headless Service also publishes not-ready addresses, so it kept a target pointed at a pod whose port is still closed during boot and reported it as down. The ServiceMonitor and the ClusterIP Service now share a `openwa.io/scrape-target` label that the headless Service does not carry. After upgrading, series previously collected through the headless target stop appearing: check any dashboard or alerting rule that matches on the `service` label, which now always resolves to the ClusterIP Service.
- The Helm chart no longer restarts the pod while it is still starting. The container binds its port only after every bootstrap hook has finished, so during boot the endpoint is closed rather than unhealthy and each liveness probe is a connection refusal; with no startup probe the liveness settings governed that window and allowed 50 seconds before the kubelet killed the container. This is not limited to deployments using session auto-start: the database connect retry alone is up to 10 attempts three seconds apart, so a database that is slow to accept connections can consume 30 of those 50 seconds on a stock install. A startup probe now allows 300 seconds (a 5-second period, 60 failures) and fires immediately, so a fast boot is recognised in about five seconds rather than after a fixed delay — the pod becomes live sooner than before, not later. A boot that is genuinely unbounded is not made safe by any fixed budget.
- A Helm upgrade that changes only `env` or `secretEnv` now restarts the pod, so the new configuration actually reaches the running container. The pod template carried nothing derived from the ConfigMap or Secret, so such an upgrade rendered a byte-identical template and triggered no rollout: the release reported success while the process kept its previous values, because Kubernetes reads `envFrom` only when a container is created. The pod template now carries checksums of the rendered ConfigMap and Secret. Two consequences worth reading before upgrading: a configuration change now restarts this single-instance StatefulSet where previously it did not, and the chart can only observe configuration it renders itself — with `existingSecret` set no Secret is created, so changes to that operator-managed Secret still need `kubectl rollout restart`. The Secret checksum is published as a pod annotation, which is readable by anyone who can list pods; `values.yaml` notes what that means when choosing between `secretEnv` and `existingSecret`.
...
- Auto-starting previously authenticated sessions no longer delays the HTTP listener. NestJS binds the port only after every bootstrap hook has finished, and the auto-start loop launches one engine at a time with a 2s throttle, each bounded only by the engine-init timeout of at least 60 seconds — so a host with several sessions kept the port closed, not merely unhealthy, for minutes. The Helm chart's liveness budget is roughly 50 seconds and the Docker `HEALTHCHECK` encodes the same expectation, so a restart could be killed before it ever answered, and no probe budget can cover a bound that grows with the session count. The launches now run after the port is open; a shutdown that lands mid-run stops the remaining launches and waits for the one in flight. A failure of the session scan itself is now logged instead of aborting startup.
